    ...
    <main className="min-h-screen bg-[#0c0b0a] text-[#efe9df] overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 pt-40 pb-28">
        ...
        <h1 className="text-4xl md:text-6xl font-bold font-syne uppercase tracking-tight text-[#f2ede3] leading-[1.05] max-w-4xl">
          Job Application
          <br />
          <span className="text-[#f2ede3]">Tailor</span>
        </h1>
        ...
        <div className="mt-5 grid md:grid-cols-2 gap-5">
          <div>
            <label className="block mb-2 text-[#8a8175] font-syne text-[10px] uppercase tracking-widest">
              Your resume
            </label>
            <textarea
        ...
        {result && (
          <div className="mt-20 space-y-8">
            {/* Score + verdict */}
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-10 shadow-[0_8px_30px_rgba(15,23,42,0.03)]">
              <p className="text-[#f2ede3] uppercase tracking-[0.2em] text-xs font-semibold">
                ATS Match Score
              </p>
              <h2 className={`mt-4 text-6xl font-bold font-syne ${scoreColor}`}>
                {result.atsScore}
                <span className="text-3xl text-[#6f675b]">/100</span>
              </h2>
              <p className="mt-4 text-[#8a8175] text-lg leading-relaxed max-w-2xl">
                {result.verdict}
              </p>

              <div className="mt-8 grid md:grid-cols-2 gap-8">
                <div>
                  <h4 className="text-[#e4dece] font-semibold mb-3 text-sm uppercase tracking-wide">
                    Matched
                  </h4>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-10 shadow-[0_4px_20px_rgba(15,23,42,0.02)]">
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="text-[#8a8175] hover:text-[#f2ede3] font-syne text-[10px] uppercase tracking-widest transition-colors"
                >
                  {copied === "resume" ? "Copied ✓" : "Copy"}
                </button>
              </div>
              <p className="mt-6 text-[#a89f8f] leading-relaxed">
                {result.tailoredSummary}
              </p>
              <ul className="mt-6 space-y-3">
                {result.tailoredBullets?.map((b, i) => (
                  <li key={i} className="flex gap-3 text-[#a89f8f] leading-relaxed">
                    <span className="text-[#f2ede3] mt-1">▸</span>
                    <span>{b}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-10 shadow-[0_4px_20px_rgba(15,23,42,0.02)]">
              ...
              <p className="mt-6 text-[#a89f8f] leading-relaxed whitespace-pre-wrap">
                {result.coverLetter}
              </p>
            </div>
            ...
            {result.tips?.length > 0 && (
              <div className="rounded-3xl border border-white/10 bg-white/[0.04]/50 p-8">
                <h4 className="text-xl font-semibold mb-4 text-[#e4dece] font-syne uppercase tracking-tight">
